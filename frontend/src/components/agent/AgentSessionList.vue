...
import {Loader2, MessageSquare, Pencil, Pin, Plus, Search, Trash2} from '@lucide/vue'
import {useAgentChatStore} from '@/stores/agent-chat'
import {useAgentStore} from '@/stores/agent'
import type {SessionVO} from '@/types/session'
import {useAgentSessionList} from '@/composables/useAgentSessionList'
...
async function selectSession(session: SessionVO) {
  await agentChatStore.loadConversation(session.id)
}
</script>
...
<template>
  <div class="session-section">
      ...
      <div :style="{ height: totalHeight + 'px', position: 'relative' }">
        ...
        <div :style="{ transform: `translateY(${offsetY}px)` }">
          <div
              ...
              :key="session.id"
              class="session-item"
              :class="{ active: agentChatStore.currentConversationId === String(session.id) }"
    ...
    <!-- 新对话按钮：仅重置前端状态，首次发送消息时才创建后端 session -->
    <button class="new-chat-btn" :disabled="!agentStore.activeId"
            @click="agentChatStore.newConversation()">
