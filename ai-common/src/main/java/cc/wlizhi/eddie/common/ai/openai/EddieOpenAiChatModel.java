...
public class EddieOpenAiChatModel implements ChatModel {
    ...
    private final EddieOpenAiChatOptions options;
     ...
     */
    public static EddieOpenAiChatModel.Builder builder() {
        return new EddieOpenAiChatModel.Builder();
    }

    private EddieOpenAiChatModel(OpenAIClient openAiClient, OpenAIClientAsync openAiClientAsync, EddieOpenAiChatOptions options,
     ...
     */
    @Override
    public EddieOpenAiChatOptions getOptions() {
        return this.options;
    }
    ...
    ChatCompletionCreateParams createRequest(Prompt prompt, boolean stream) {
        ...
        EddieOpenAiChatOptions requestOptions = (EddieOpenAiChatOptions) prompt.getOptions();
    ...
    private static final class ChunkMerger {
        ...
        static boolean toolCallsDone(ChatCompletionChunk chunk) {
            return !chunk.choices().isEmpty()
                    && ChatCompletionChunk.Choice.FinishReason.TOOL_CALLS == chunk.choices().get(0).finishReason().orElse(null);
        }
        ...
        static ChatCompletionChunk mergeChunks(List<ChatCompletionChunk> chunks) {
            ...
            Map<Long, ChatCompletionChunk.Choice> choices = new LinkedHashMap<>();
        ...
        private static ChatCompletionChunk.Choice mergeChoices(ChatCompletionChunk.Choice c1, ChatCompletionChunk.Choice c2) {
        ...
        private static ChatCompletionChunk.Choice.Delta mergeDeltas(ChatCompletionChunk.Choice.Delta left, ChatCompletionChunk.Choice.Delta right) {
            var tcs = Stream.of(left.toolCalls(), right.toolCalls()).flatMap(Optional::stream).reduce((tcs1, tcs2) -> {
                ...
                ChatCompletionChunk.Choice.Delta.ToolCall toolCall = tcs2.get(0);
                if (toolCall.id().isPresent()) {
                    ...
                    var concatenatedArgs = Stream
                            .of(lastFromTc1F.arguments(), toolCall.function().flatMap(ChatCompletionChunk.Choice.Delta.ToolCall.Function::arguments))
    ...
    public static class ResponseFormat {
        ...
        private ResponseFormat.Type type = ResponseFormat.Type.TEXT;
        ...
        public void setType(ResponseFormat.Type type) {
            this.type = type;
        }
        ...
        public static final class Builder {
            ...
            public ResponseFormat.Builder type(ResponseFormat.Type type) {
                this.responseFormat.setType(type);
                return this;
            }

            public ResponseFormat.Builder jsonSchema(String jsonSchema) {
                this.responseFormat.setType(ResponseFormat.Type.JSON_SCHEMA);
    ...
    public static final class Builder {
        ...
        private @Nullable EddieOpenAiChatOptions options;
         ...
         * @return this builder
         */
        public Builder options(EddieOpenAiChatOptions options) {
            this.options = options;
            return this;
        }
         ...
         */
        public EddieOpenAiChatModel build() {
            EddieOpenAiChatOptions resolvedOptions = this.options != null ? this.options
                    : EddieOpenAiChatOptions.builder().build();
            ...
            return new EddieOpenAiChatModel(resolvedClient, resolvedClientAsync, resolvedOptions,
