...
1. 步骤描述应上下文自描述：每个步骤的 description 写清楚三要素——输入（通常来自 depends_on
   指向的步骤的输出）、操作、产出。执行模型拿到单条步骤时，即使不翻看其他步骤，也能明确知道要做什么。
2. 完成标志明确：goal 字段应写明可验证的验收标准（如 "已输出各品类的月销量趋势图和 Top10 排行"），而非模糊描述（如 "
   完成数据分析"）。
...
| 字段                   | 类型        | 说明                                         |
|----------------------|-----------|--------------------------------------------|
| step_number          | integer   | 步骤序号，从1开始递增                                |
