 ...
 * 智谱AI 远程模型获取器
 * <p>
 * 智谱AI API 兼容 OpenAI 格式，但路径版本为 v4：
 * <pre>
 * GET {baseUrl}/v4/models
 * {
 *   "object": "list",
 *   "data": [
 ...
 * }
 * </pre>
 * 文档：<a href="https://open.bigmodel.cn/dev/api">https://open.bigmodel.cn/dev/api</a>
 * <p>
 * 注意：智谱AI 的 OpenAI 兼容路径为 /v4 而非 /v1。
 */
@Component
public class ZhipuModelFetcher implements RemoteModelFetcher {

    private static final String MODELS_PATH = "/v4/models";
    ...
    private List<ModelVO> parseResponse(String json) {
        try {
            ...
            throw new RuntimeException("解析智谱AI远程模型列表返回数据失败: " + e.getMessage(), e);
        }
    }
}
